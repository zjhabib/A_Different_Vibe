...
import itertools
import tweepy
from app.api.twitter import twitter_api
...
#Export all tweets to csv file for migrate into db. Comment out when no need for large migration
# all_tw_df.to_csv ('twitter_api.csv', index = False, header=True)

twitter_db = pd.read_csv(twitter_api)

def update_tweets_db(twitter_db, api_tweets):
    for tweet in api_tweets:
        if tweet['tweetID'] not in twitter_db.tweetID:
            twitter_db.append(tweet)